...
# Service Goods Phillips Curve
F[indexes.mc] =
    (log(πS) - XSS[indexes.πSSS]) - (
        m_par.κ * (mc - 1 / m_par.μ) +
        m_par.β * ((log(πSPrime) - XSS[indexes.πSSS]) * SPrime / S)
    )

# Rate of return on capital
F[indexes.RK_before_taxes] =
    (log(RK_before_taxes)) -
    (log(1.0 + interest(Z, H, S, m_par) + m_par.δ_0 - q * m_par.δ_0))
# This variable needs to be set for the package!
...
F[indexes.wF] = (log(wF)) - (log(wage(mc, Z, H, S, m_par)))

# Service Goods Firm profits
F[indexes.Π_S] = (log(Π_S)) - (log((1.0 .- mc) .* Y .* (1 - m_par.α)))

# Housing Firm profits (auxiliary, cannot be a variable because of zero steady state)
Π_H = q * (KPrime - (1.0 - m_par.δ_0) * K) - I

# Distributed profits to entrepreneurs
F[indexes.Π_E] = (log(Π_E)) - (log(Π_H + Π_S))
# This variable needs to be set for the package!
...
# Production function
F[indexes.Y] = (log(Y)) - (log(output(H, S, m_par)))

# Housing Definition
F[indexes.H] = (log(H)) - (log(K) + log(Z))

# Services Definition
F[indexes.S] = (log(S)) - (log(N) + log(Z))

# TFP
F[indexes.Z] = (log(ZPrime)) - (m_par.ρ_Z * log(Z))

# Price Inflation
F[indexes.πS] = (log(π)) - (log(πS) * m_par.α + log(πK) * (1.0 - m_par.α))

# Housing Inflation
F[indexes.πK] = (log(πK)) - (log(RKPrime + m_par.δ_0) - log(RK + m_par.δ_0))
