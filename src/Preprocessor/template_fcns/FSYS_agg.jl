...
file and copies the content from `input_aggregate_model.mod` into the code block below at
...
`indexes` can be both `IndexStruct` or `IndexStructAggr`; in the latter case (which is how
function is called by [`LinearSolution_reduced_system()`](@ref)), variable-vectors
...
function Fsys_agg(
    ...
    XSS::Array{Float64,1},
    distr::Union{AbstractArray,RepAgent,CDF,CopulaOneAsset,CopulaTwoAssets},
    m_par,
    n_par,
    indexes,
)
