...
Consult the document ['ComputationalNotes.md'](ComputationalNotes.md), specifically section
3 with the title 'Update the continuation values (CV1) and (CV2) using the Envelope
...
function updateW(
    ...
    m_par,
    n_par,
    beta_factor::Number = 1.0,
)

    # Preallocate variables
    vf = similar(Evf)
    mutil_x_a = similar(Evf.b)

    # Call the inplace function to update the marginal value functions
    updateW!(vf, mutil_x_a, Evf, pf, args_hh_prob, m_par, n_par, beta_factor)
...
function updateW!(
    ...
    nb, nk, nh = size(pf.x_n_star)
    β::Float64 = m_par.β * beta_factor
    ...
    mutil!(mutil_x_a, pf.x_a_star, m_par)

    # Probability-weighted sum of marginal value of liquid assets
    vf.b .= m_par.λ .* mutil_x_a .+ (1.0 - m_par.λ) .* vf.b
    vf.b .= vf.b ./ (1.0 .+ (Tc .- 1.0))
    ...
    vf.k .=
        (RK .- 1.0) .* vf.b .+ m_par.λ .* q .* mutil_x_a ./ (1.0 .+ (Tc .- 1.0)) .+
        (1.0 .- m_par.λ) .* β .* vf.k
end
