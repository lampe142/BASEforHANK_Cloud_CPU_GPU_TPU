...
"""
function MakeTransition(pf::PolicyFunctionsTwoAssets, Π::Array{Float64,2}, n_par)
    ...
    j_k_a, ω_left_k_a, ω_right_k_a = MakeWeights(pf.k_a_star, n_par.grid_k)
    j_b_a, ω_left_b_a, ω_right_b_a = MakeWeights(pf.b_a_star, n_par.grid_b)
    j_b_n, ω_left_b_n, ω_right_b_n = MakeWeights(pf.b_n_star, n_par.grid_b)

    # Create the containers of the output for the adjustment case
    weight_adj =
        Array{eltype(pf.k_a_star),3}(undef, 4, n_par.nh, n_par.nk * n_par.nb * n_par.nh)
    ...
    weight_non = zeros(eltype(pf.k_a_star), 2, n_par.nh, n_par.nk * n_par.nb * n_par.nh)
...
function MakeTransition(pf::PolicyFunctionsOneAsset, Π::Array{Float64,2}, n_par)
    ...
    # Create the containers of the output for the non adjustment case
    weight_non = zeros(eltype(pf.b_n_star), 2, n_par.nh, n_par.nb * n_par.nh)
    targetindex_non = zeros(Int, 2, n_par.nh, n_par.nb * n_par.nh)
    startindex_non = zeros(Int, 2, n_par.nh, n_par.nb * n_par.nh)
    ...
    for hh = 1:(n_par.nh) # all current income states
        for bb = 1:(n_par.nb) # all current liquid asset states
            ...
            # Calculate the weights for the two possible transitions
            ωL = ω_left_b_n[bb, hh]
            ωR = ω_right_b_n[bb, hh]

            # Calculate the target index, which policy functions point towards, using linear
            # indexing Note: The index is calculated by adding the linear index of the
            # liquid asset policy function to the linear index of the illiquid asset holding
            # times the number of liquid asset states
            j_non = j_b_n[bb, hh]

            # iterating over all future income states
            for jj = 1:(n_par.nh)
                # extract the probability of transitioning from the current income state to
                # the future income state
                pp = Π[hh, jj]
